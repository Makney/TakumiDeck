...
export class ModelStatsRepository {
  ...
  getModelsBreakdown(input: {
    ...
    let tokensTotal = 0;
    for (const r of raw) tokensTotal += r.tokens;
...
export class InMemoryModelStatsDriver implements ModelStatsDbDriver {
  ...
  modelBreakdown(q: ModelStatsQuery): ModelStatsRawRow[] {
    const byModel = new Map<
      string,
      { messages: number; sessions: Set<string>; tokens: number }
    >();
    for (const m of this.messages) {
      ...
      if (!entry) {
        entry = { messages: 0, sessions: new Set(), tokens: 0 };
    ...
    for (const [model, e] of byModel) {
      rows.push({ model, messages: e.messages, sessions: e.sessions.size, tokens: e.tokens });
    }
