...
| `Terminal-Session ohne Claude`       | ✅      | 2026-05-24 (Phase-2-Season-31) — Sechster `SessionType: 'terminal'` spawnt `pwsh.exe` (mit `powershell.exe`-Fallback) statt der claude-Binary; kein `--session-id`/`--model`, kein JSONL, kein Token-Verbrauch. Fuenf Inline-Gates im `pty:create`-Handler skippen die claude-spezifischen Pfade (Pre-Spawn-Check via `claude_binary_path`, `jsonl_path`-Berechnung, `claude_session_id` + `current_model` in der DB, `pollingRing.attach`). Resume-Pfad in `session:resume` analog. Neuer Pure-Helper `src/main/pty/terminalShell.ts` mit zweistufigem PATH-Lookup. Modal blendet Modell-Dropdown + On-Demand-Kontext-Block bei `type='terminal'` aus; Submit schickt Sentinel `'none'` (zod-Pflicht erfuellen), den der Handler ignoriert. Verlauf-Filter zeigt einen eigenen „Terminal"-Bucket. 6 neue Tests (3 Shell-Resolver, 3 Repo-Vertraege bei null-Werten + Regression-Guard). |
| `User-erweiterbare Modell-Liste + Auto-Refresh` | ✅ | 2026-05-29 (Phase-2-Season-34) — Built-in-Modell-Liste in `src/shared/models.ts` (jetzt inkl. `claude-opus-4-8`) plus user-gepflegte `custom_models` im Settings-Tab „Modelle" (ID + Anzeigename + optionales Kontext-Limit); `buildModelOptions` vereint beide, ID-Kollision laesst das Custom-Label das Built-in ueberschreiben. Neuer Settings-Block „Verfuegbare Modelle abrufen" (Variante D) fragt optional den Anthropic-Endpoint `GET /v1/models` ab: nur mit gesetztem `ANTHROPIC_API_KEY` wirksam — bei Abo-/OAuth-Nutzung kommt `available=false` (Hinweis auf manuelle Pflege statt Fehler). Bekannte IDs werden via `diffNewModels` herausgefiltert, Neuzugaenge per „+ Uebernehmen" als Custom-Modell uebernommen. Kontext-Limits liefert die API nicht — bleiben Default/eigene Eingabe. Pure-Helper (`parseAnthropicModelsResponse`, `diffNewModels`) + injizierbare Netzwerk-Schicht (`src/main/models/fetchAvailableModels.ts`) + IPC `models:fetch-available`. Migration 3 setzt `custom_models: []` fuer Bestandsuser. 24 neue Tests (19 shared, 5 main mit gemocktem fetch). |
| `Terminal-Buffer-Persistierung ueber Resume` | ✅ | 2026-05-25 (Phase-2-Season-33) — Nur fuer `type='terminal'`: TerminalTab serialisiert beim Cleanup via `@xterm/addon-serialize`, trimt auf Last-2000-Zeilen + 256-KiB-Byte-Cap (Pure-Helper `trimBufferSnapshot` in `src/shared/buffer-snapshot.ts`) und schreibt den Snapshot in die neue Tabelle `session_buffer_snapshots` (Migration 0010, FK ON DELETE CASCADE auf `sessions.id`). Beim Resume-Mount laedt der Renderer den Snapshot per IPC und schreibt ihn vor dem ersten PTY-Frame in xterm — eine Pre-Frame-Queue puffert `pty:data`-Events, bis der Load-Roundtrip durch ist, damit pwshs Welcome-Zeile nicht vor dem alten Verlauf landet. Claude-Sessions skippen den Pfad komplett (claude rendert seinen Verlauf beim `--resume` selbst aus der JSONL); der `terminal:save-buffer`-Handler lehnt fremde Typen mit `TERMINAL_BUFFER_TYPE_MISMATCH` ab, `terminal:load-buffer` liefert dort still `snapshot: null`. App-Crash zwischen Tab-Sessions verliert den Buffer (Cleanup feuert nicht) — als TECH_SCHULDEN-Eintrag mit debounced-Live-Save als Aufloesungs-Pfad dokumentiert. 22 neue Tests in 3 Files. |
...
| Feature                              | Status | Bemerkung |
| ------------------------------------ | ------ | --------- |
| `Programmiersprachen-Syntax`         | ✅      | 2026-06-01 (Phase-3-Feature, vorgezogen — Season 37) — Der Editor erkennt die Sprache am Datei-Suffix und zeigt Syntax-Highlighting fuer `.js/.mjs/.cjs` · `.jsx` · `.ts/.mts/.cts` · `.tsx` · `.py/.pyi` · `.rs` · `.go` · `.json` · `.css` · `.html/.htm` (plus `.md`/`.yaml` wie bisher). Variante A: die bestehende Editor-Komponente erweitert (kein zweiter Editor) — Sprache via CodeMirror-`Compartment`. markdown + yaml liegen statisch im Bundle (kein Lazy-Flash bei den haeufigsten Files), alle weiteren Grammatiken werden beim ersten Treffer per `import()` nachgeladen (eigene Vite-Chunks) und ohne Re-Mount in die Compartment eingespielt. Auto-Indent + Bracket-Matching kommen pro Sprache ueber das `lang-*`-Paket mit. Sonderfaelle `.toml`/`.proto`/`.ini` bleiben dokumentiert Plain-Text (kein offizielles CM6-Paket). Preview-Toggle nur fuer echte Markdown-Files; Code-/YAML-/JSON-Files laufen editor-only (Toolbar ohne Preview-Pille, reuse des `td-md-body-editor`-Layouts). Neues Pure-Modul `src/renderer/components/editorLanguage.ts` (Suffix-Map + `detectLanguageId`/`isPreviewableMarkdown`/gecachter `loadLanguageExtension`), 13 neue Tests. 6 neue `@codemirror/lang-*`-Dependencies. |
