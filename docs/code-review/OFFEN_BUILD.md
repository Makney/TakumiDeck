...
## Bereichs-Review Build/Konfig (2026-05-12)

Befunde aus dem Bereichs-Review Build/Konfig (Commit `5dc33d0` + Hotfix `ecdca93`). Bewusste Design-by-Choice-Entscheidungen, mehrfach in [TECH_SCHULDEN.md](../TECH_SCHULDEN.md) und [ENTSCHEIDUNGEN.md](../ENTSCHEIDUNGEN.md) dokumentiert.
...
- **Status 2026-06-01 · Variante A umgesetzt, CI-Verifikation ausstehend:** `runs-on` in `.github/workflows/release.yml` von `windows-latest` auf `windows-2022` gepinnt (mit Begründungs-Kommentar) — das zuletzt grün gelaufene Image (v0.3.2). **Bewusst noch NICHT als behoben markiert/archiviert:** der Befund tritt nur auf dem GitHub-Runner auf, der Pin lässt sich lokal nicht verifizieren. Erst nach einem grünen CI-Lauf (per `workflow_dispatch` oder Wegwerf-Tag) als behoben markieren und archivieren. Variante B (Forge-Toolchain auf neuesten stabilen Stand heben) bleibt als nachhaltige Lösung offen.
